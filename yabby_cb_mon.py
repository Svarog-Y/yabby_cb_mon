...
def create_coupon(login, reward):
    ...
    payload = {"redeem_amount": reward, "play_through_amount": reward * 10, "can_be_referred": True,
               "limit_withdrawals": "fixed_amount", "limit_withdrawals_fixed_amount": reward * 10,
               "limit_withdrawals_multiple_deposit": 0, "limit_withdrawals_code": "string",
               "disable_all_net_progressives": True, "earnings_restricted_to_deposit": False,
               "free_spin_deposit_list": None, "previous_coupon_id": 0, "minimum_bonus_balance_type": "no_limit",
               "minimum_bonus_balance": 0, "require_deposit_fixed_amount_on_redemption": False,
               "play_through_free_spin_wins": 0, "wager_bonus": 0, "free_spins_configuration": None,
               "coupon_code": coupon_code, "date_valid_start": start_today, "date_valid_end": end_today, "user_id": 0,
               "method_list": "string", "redeem_zero": 0, "redeem_verified_email": True, "redeem_new_players": False,
               "comment": comment, "associated_affiliate_id": None, "redeem_for_affiliate_id": None,
               "schedule_days_of_week": None, "schedule_start_time": 0, "schedule_duration_minutes": 3599,
               "published": False, "allow_redemption_from": ["download"], "allowed_skin_list": [],
               "exclude_player_class": None, "player_filters": {"days_since_sign_up": {"type": "string", "value": {}},
                                                                "days_since_birthday": {"type": "string", "value": {}},
                                                                "days_since_first_deposit": {"type": "string",
                                                                                             "value": {}},
                                                                "days_since_last_deposit": {"type": "string",
                                                                                            "value": {}},
                                                                "countries_to_exclude": [], "exclude_coupons": [],
                                                                "currencies": ["USD", "AUD", "EUR"]}, "sim_percent": 0,
               "count_days_deposit": 0, "count_days_play": 0, "game_play_through_list": []}

    while True:  # attempt multiple times if coupon code exists
        print(f"-- create {coupon_code}...")
        r = requests.post(url=main_url + "api/v2/coupons/bonus-balance/single-use/fixed-amount-on-redemption",
                          json=payload, cert=cert_path)
        if r.status_code == 200:
            print(f"----> success!")
...
def redeem_coupon(pid, coupon_code):
    ...
    r = requests.post(url=main_url + f'api/v2/players/{pid}/coupons',
                      params={'couponCode': coupon_code, 'redeem': True, 'ignorePlayerRestrictions': True},
                      cert=cert_path)
    if r.status_code == 200:
        print("----> success!")
        return 1
...
terms = ["====================================================", "Terms setup:", f"Minimum Loss: ${min_loss}",
         f"Minimum Deposits: ${min_deposits}", f"Minimum Reward: ${min_reward}",
         f"Maximum WDs/DEPs Ratio: {max_ratio}%"]
for item in terms:
    print(item)
...
for n in range(5):  # attempt 5 times
    ...
    if r.status_code == 200:
        ...
        df1 = df1.drop(
            ['casino_name', 'last_name', 'day_phone_number', 'evening_phone_number', 'full_address', 'address_2',
             'city', 'country', 'zip_code', 'net_cash_position', 'signup_date', 'signup_skin', 'last_game_date',
             'birth_date'], axis=1)  # drop unnecessary columns
...
for index, row in df1.iterrows():  # iterate through dataframe rows [depositors]
    ...
    if player_class == 'Bonus Abuser' or player_class == 'Bonus Hunter' or player_class == 'New':
        if player_class == "New":
            player_class = "Novice Customer"  # for better reporting
    ...
    if deposits < min_deposits:
        print(
            f'[{current}/{total}] {login} ineligible - deposits [${round(deposits, 2)}] below minimum [${min_deposits}].')
        df1.at[index, 'eligible'] = False
        continue
    if loss < min_loss:
    ...
    if ratio > max_ratio:
        print(
            f'[{current}/{total}] {login} ineligible - withdrawals ratio [{round(ratio, 2)}%] above maximum [{max_ratio}%].')
        df1.at[index, 'eligible'] = False
        continue

    # calculate reward
    if player_class.casefold() == 'Rookie'.casefold():
        reward = rookie_reward * loss
    elif player_class.casefold() == 'Pro'.casefold():
        reward = pro_reward * loss
    elif player_class.casefold() == 'Star'.casefold():
        reward = star_reward * loss
    elif player_class.casefold() == 'Prestige'.casefold():
        reward = prestige_reward * loss
    elif player_class.casefold() == 'Hall of Fame'.casefold():
        reward = hof_reward * loss
    ...
    if coupon_code == False:  # if failed, put Event Log Comment and send email
        ...
        print(
            f"[{current}/{total}] {login} - failed to create a coupon so support will have to cover it. Reward amount "
            f"noted on customer's event log, 'pending reward' email sent.")
        ...
        if r == 0:  # if soft failure
            ...
            send_email(2, row['first_name'], row['email'])
            print(
                f"[{current}/{total}] {login} - failed to redeem coupon so support will have to cover it. Coupon "
                f"created, noted on customer's event log, 'pending reward' email sent.")
        elif r == 1:  # processed
            df1.at[index, 'processed'] = True
            send_email(1, row['first_name'], row['email'])
            print(f"[{current}/{total}] {login} - success (Reward Processed)!")
